# Limitless Search 插件开发指南

## 概述

Limitless Search 采用异步插件架构，支持通过插件扩展搜索来源。插件系统基于 Go 接口设计，提供高性能的并发搜索能力和智能缓存机制。
...
Limitless Search支持插件级别的Service层过滤控制，允许插件自主决定是否在Service层进行关键词过滤：
...
Limitless Search 采用4级插件优先级系统，直接影响搜索结果的排序权重：
...
插件优先级在Limitless Search的多维度排序算法中占据主导地位：
...
**Limitless Search系统的智能处理**:

Limitless Search 会根据消息中的链接数量自动决定是否提取 work_title：
...
// Limitless Search会根据消息文本自动为每个链接提取work_title
```
...
Limitless Search系统支持以下网盘类型的自动识别（完整列表）：
...
Limitless Search 支持插件注册自定义的 HTTP 路由，用于实现插件专属的管理页面、配置接口或其他Web功能。
...
Limitless Search 在启动时会自动扫描并注册所有实现了 `PluginWithWebHandler` 接口的插件路由：
