# Limitless Search MCP 服务文档

## 功能介绍

Limitless Search MCP 服务是一个基于 [Model Context Protocol (MCP)](https://modelcontextprotocol.io) 的工具服务，它将 Limitless Search 网盘搜索 API 的功能封装为可在支持 MCP 的客户端（如 Cherry Studio）中直接调用的工具。

通过 Limitless Search MCP 服务，可以直接在 Claude 等 AI 助手中搜索网盘资源，极大地提升了获取网盘资源的便捷性。
...
2. **检查服务健康状态 (`check_service_health`)**:
   - 检查所连接的 Limitless Search 后端服务是否正常运行。
   - 获取后端服务的配置信息，如可用的 Telegram 频道列表和插件列表。

3. **启动后端服务 (`start_backend`)**:
   - 自动启动本地的 Limitless Search Go 后端服务（如果尚未运行）。
...
Limitless Search MCP 服务设计为与 Limitless Search Go 后端服务分离，通过 HTTP API 进行通信。支持以下部署方式：

- **Node.js 部署 (TypeScript)**: MCP 服务基于 TypeScript 开发，编译后通过 `node` 命令运行编译后的 JavaScript 文件。它会自动连接到指定的 Limitless Search 后端服务。
- **Docker 部署**: 使用 Docker 容器运行 Limitless Search 后端服务，MCP 服务通过 HTTP API 连接到容器化的后端。
...
Limitless Search 后端服务通常运行在 `http://localhost:8888` (默认地址)。支持以下两种后端部署方式：
...
- 克隆或确保已有 Limitless Search Go 项目源码。
...
在 Limitless Search 项目根目录下，使用 Docker Compose 启动后端服务：
...
- **使用 `node` 直接运行** (手动启动):
  在 Limitless Search 项目根目录下（包含 `typescript` 文件夹），运行：
...
服务启动后，将默认尝试连接到 `http://localhost:8888` 的 Limitless Search 后端服务。
...
要在 Cherry Studio 中使用 Limitless Search MCP 服务，需要将其添加到 Cherry Studio MCP 的配置文件中。
...
配置完成后，在对话界面启用 Limitless Search MCP 服务，即可开始尝试搜索。
...
| 环境变量               | 描述                                                       | 默认值                    |
| :--------------------- | :--------------------------------------------------------- | :------------------------ |
| `PANSOU_SERVER_URL`    | Limitless Search 后端服务的 URL 地址。                               | `http://localhost:8888`   |
...
| `PROJECT_ROOT_PATH`    | Limitless Search 后端可执行文件所在的目录路径（用于自动启动）。      | 无                        |
